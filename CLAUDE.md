...
CI 통과 → main 머지 → Vercel/Railway 자동 배포 → Vercel MCP `list_deployments`로 READY 확인 (§7).
...
1. **외부 API는 코드 전에 실호출.** 스키마만 맞추고 넘어가지 않는다. (§3)
2. **"동작할 것이다"는 검증이 아니다.** 실제 출력 보여준다. (§4)
